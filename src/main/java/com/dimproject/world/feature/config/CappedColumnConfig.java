...
public class CappedColumnConfig implements FeatureConfiguration {
    public static final Codec<CappedColumnConfig> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ...
            Codec.BOOL.optionalFieldOf("ignore_placement_condition", false).forGetter(config -> config.placementCondition)
    ...
    public final boolean placementCondition;

    public CappedColumnConfig(BlockStateProvider stemBlock, BlockStateProvider capBlock, int minStemSize, int maxStemSize, boolean uniformHeightDistribution, boolean upsideDown, boolean placementCondition) {
        super(); // FeatureConfiguration n’a pas de paramètre par défaut
    ...
    public static class Builder {
        ...
        public CappedColumnConfig build() {
            return new CappedColumnConfig(stemBlock, capBlock, minStemSize, maxStemSize, uniformHeightDistribution, upsideDown, uniformHeightDistribution);
        }
    }
}